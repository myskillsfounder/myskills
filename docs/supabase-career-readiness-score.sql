...
--
-- THE METHOD (v2) — the same rules as src/lib/readinessScore.ts, which stays
-- as the student's "what next" guide and as a fallback if this isn't run:
--   Personal development  30   2 per module finished (5 x 2) + 20 when a mentor
--                              signs off the programme
--   Professional          30   highest VERIFIED education level (Class 10 = 4,
--                              Class 12 = 8, Diploma = 12, Bachelor's = 16,
--                              Master's = 20, Doctorate = 22; x0.75 while in
--                              progress) + 1 per skill listed, up to 8
--   Experience            40   5 per verified internship (max 15) + 1 per
--                              verified month of other work (max 15) + 2.5 per
--                              verified project (max 10)
-- Education, experience and projects count only while identity is verified and
-- the entry still matches what was checked. Age, institution and MySkills test
-- scores are never used.
--
-- VERIFIED vs SELF-REPORTED: education, experience, projects and the mentor
-- sign-off are verified by a person. Skills (self-declared) and module
-- completions (written answers no one has read yet) are self-reported. The
-- score adds both, and reports the two totals separately so nobody has to
-- guess how much of a number was checked.
...
as $$
declare
  c_method constant text := 'v2';
  c_mod_pts constant int := 2;
  c_mentor_pts constant int := 20;
  ...
  v_self numeric;
begin
  select p.full_name, p.date_of_birth::text, p.education, p.experience, p.skills
    into v_name, v_dob, v_edu, v_exp, v_skills
  ...
  if v_identity then
    ...
    for r in select value as e from jsonb_array_elements(coalesce(v_edu, '[]'::jsonb)) loop
      ...
      then
        ...
        v_pts := case v_lvl
                   when 'class-10' then 4 when 'class-12' then 8 when 'diploma' then 12
                   when 'bachelors' then 16 when 'masters' then 20 when 'doctorate' then 22
                   else 0 end;
  ...
  v_skill_pts := least(jsonb_array_length(coalesce(v_skills, '[]'::jsonb)), 8);

  -- Personal development: modules with all four items written, and the sign-off.
  ...
  v_edu_total := v_best_edu + v_skill_pts;
  v_exp_total := least(v_internships * 5, 15) + least(v_work_months, 15) + least(v_projects * 2.5, 10);
  v_personal  := least(v_modules, 5) * c_mod_pts + case when v_mentor then c_mentor_pts else 0 end;

  v_verified := v_best_edu + v_exp_total + case when v_mentor then c_mentor_pts else 0 end;
  v_self     := v_skill_pts + least(v_modules, 5) * c_mod_pts;

  return jsonb_build_object(
    'method_version', c_method,
    'score', round(v_personal + least(v_edu_total, 30) + least(v_exp_total, 40))::int,
    'verified_points', round(v_verified, 1),
    'self_reported_points', round(v_self, 1),
    'identity_verified', v_identity,
    'personal', jsonb_build_object(
      'points', v_personal, 'max', 30, 'modules_done', least(v_modules, 5), 'mentor_approved', v_mentor),
    'professional', jsonb_build_object(
      'points', least(v_edu_total, 30), 'max', 30, 'education_points', v_best_edu,
      'education_level', v_edu_label, 'skills_points', v_skill_pts),
    'experience', jsonb_build_object(
      'points', least(v_exp_total, 40), 'max', 40, 'internships', v_internships,
