  ...
  return (
      ...
      title={
        <div className="flex items-start gap-2.5">
          <div className="min-w-0">
            <span className="rounded-full bg-ink-200 px-2 py-0.5 text-[11px] font-medium text-ink-600">
              {trackName}
            </span>
            <h2 className="mt-1.5 text-[16px] font-semibold leading-snug text-ink-900">{item.title}</h2>
            {item.subtitle && (
              <p className="mt-0.5 text-xs leading-relaxed text-ink-600">{item.subtitle}</p>
            )}
          </div>
          <button
            type="button"
            ...
            className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-full transition-colors ${
              saved ? 'text-amber-500' : 'text-ink-400 hover:text-ink-600'
            }`}
          >
      ...
      footer={
        <div className="flex items-center gap-2">
          ...
          <a
            ...
            className="inline-flex h-11 items-center gap-1.5 rounded-full border border-ink-300 px-3.5 text-xs font-medium text-ink-600"
          >
    ...
    >
      ...
      <div className="mb-4">
              ...
              <button
                ...
                className={`shrink-0 rounded-full border px-3.5 py-2 text-xs font-medium transition-colors ${
                  active
                    ...
                    : 'border-ink-300 bg-white text-ink-600 active:bg-ink-100'
                }`}
              >
                {s.name}
              </button>
        ...
        {style && <p className="mt-2 text-xs leading-relaxed text-ink-600">{style.blurb}</p>}
      </div>
      )}

      <div className="rounded-xl border border-ink-200 bg-ink-100 p-4">
