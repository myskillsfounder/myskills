...
function trackStatus(started: boolean, percent: number) {
  if (!started) return { label: 'Not started', pill: 'border-ink-300 bg-ink-100 text-ink-600', bar: 'bg-ink-300' }
  ...
  return (
    <div>
      <div className="mb-4 flex items-baseline justify-between">
        <h2 className="font-display text-lg font-semibold text-ink-900">Skill tracks</h2>
        <span className="text-xs text-ink-500">Practice as often as you like — best score is kept</span>
      </div>
            ...
            >
              ...
              <div className="mt-4">
                <div className="flex items-center justify-between text-xs">
                  <span className="text-ink-500">{started ? 'Best score' : `${questions.length} scenarios`}</span>
